...
std::unique_ptr<IRenderContext> RenderContext_GdiPlus::Clone()
{
	std::unique_ptr<IRenderContext> pClone = std::make_unique<RenderContext_GdiPlus>();
...
UiRect RenderContext_GdiPlus::MeasureText(const std::wstring& strText, int iFont, UINT uStyle, int width /*= DUI_NOSET_VALUE*/)
{
	Gdiplus::Graphics graphics(m_hDC);
	Gdiplus::Font font(m_hDC, GlobalManager::GetFont(iFont));
