...
std::unique_ptr<IRenderContext> GlobalManager::CreateRenderContext()
{
	std::unique_ptr<IRenderContext> p;
	p.reset(m_renderFactory->CreateRenderContext());
	return p;
}

std::unique_ptr<IPen> GlobalManager::CreatePen(DWORD color, int width /*= 1*/)
{
	std::unique_ptr<IPen> p;
	p.reset(m_renderFactory->CreatePen(color, width));
	return p;
}

std::unique_ptr<IBrush> GlobalManager::CreateBrush(DWORD color)
{
	std::unique_ptr<IBrush> p;
	p.reset(m_renderFactory->CreateBrush(color));
	return p;
}

std::unique_ptr<IMatrix> GlobalManager::CreateMatrix()
{
	std::unique_ptr<IMatrix> p;
	p.reset(m_renderFactory->CreateMatrix());
	return p;
}

std::unique_ptr<IPath> GlobalManager::CreatePath()
{
	std::unique_ptr<IPath> p;
