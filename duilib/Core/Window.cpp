...
bool Window::IsCaptureControl(const Control* pControl)
{
...
IRenderContext* Window::GetRenderContext() const
{
...
void Window::OnInitLayout()
{
	if (m_pRoot && IsWindowsVistaOrGreater()) {
		...
		m_nAlpha = 0;
		std::function<void(int)> playCallback = Bind(&Window::SetAlpha, this, std::placeholders::_1);
